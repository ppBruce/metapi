...
import {
  ProxyLogDebugSettingsFooter,
  ProxyLogDebugSettingsForm,
} from '../components/ProxyLogDebugSettingsForm.js';
...
import {CompactSummaryMetric, copyTextToClipboard, renderProxyLogClientCell, ProxyLogTimingCell, StreamModeIcon} from './helpers/proxyLogsUi.js';
import {
  renderTraceStatusBadge,
  ProxyDebugTraceDetailPanel,
...
export default function ProxyLogs() {
  ...
  const debugSettingsFooter = (
    <ProxyLogDebugSettingsFooter
      ...
      onSave={() => void handleSaveDebugSettings()}
      saving={debugPanelSaving}
    />
  );
  const debugSettingsForm = (
    <ProxyLogDebugSettingsForm
      settings={debugDraftSettings}
      onChange={setDebugDraftSettings}
      isMobile={isMobile}
    ...
    <div className="animate-fade-in proxy-logs-page">
      ...
      {isMobile ? (
        ...
        >
          <div style={{ padding: 16, display: 'grid', gap: 16 }}>
            {<ProxyDebugTraceDetailPanel
            selectedDebugTraceId={selectedDebugTraceId}
            detail={selectedDebugTraceDetail}
            onCopyStoredDebugValue={handleCopyStoredDebugValue}
          />}
          </div>
        </MobileDrawer>
      ) : (
