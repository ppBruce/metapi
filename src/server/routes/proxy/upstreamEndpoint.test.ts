...
const { CODEX_CLI_USER_AGENT } = await import('../../shared/codexClientFamily.js');
...
describe('resolveUpstreamEndpointCandidates', () => {
  ...
  it('keeps openai platform messages-first when the catalog only advertises generic openai/chat support', async () => {
    fetchModelPricingCatalogMock.mockResolvedValue({
      models: [
        {
          modelName: 'claude-opus-4-6',
...
describe('buildUpstreamEndpointRequest', () => {
  ...
  it('builds codex responses requests against backend-api path and preserves oauth provider headers', () => {
    ...
    expect(request.headers.Version).toBe('0.153.4');
    ...
    expect(request.headers['User-Agent']).toBe(CODEX_CLI_USER_AGENT);
    expect(request.headers.Accept).toBe('application/json');
